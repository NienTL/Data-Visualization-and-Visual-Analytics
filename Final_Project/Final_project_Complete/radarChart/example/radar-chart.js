      ...
      selection.each(function(data) {
        ...
        if (cfg.levelTick){
          ...
          .attr('transform', function(levelFactor) {
            return 'translate(' + (cfg.w/2-levelFactor) + ', ' + (cfg.h/2-levelFactor + cfg.verticalOffset) + ')';
          });
        }
        else{
          levelLine
        ...
        if(cfg.axisLine || cfg.axisText) {
          ...
          if(cfg.axisLine) {
            axis.select('line')
            .attr('x1', cfg.w/2)
            .attr('y1', cfg.h/2 + cfg.verticalOffset)
            .attr('x2', function(d, i) { return (cfg.w/2-radius2)+getHorizontalPosition(i, radius2, cfg.factor); })
            .attr('y2', function(d, i) { return (cfg.h/2-radius2)+getVerticalPosition(i, radius2, cfg.factor) + cfg.verticalOffset; });
          }
          ...
          if(cfg.axisText) {
            ...
            .attr('dy', function(d, i) {
              ...
              return ((p < 0.1) ? '0em' : ((p > 0.9) ? '0.5em' : '0.5em'));
            })
            .text(function(d) { return d.name; })
            .attr('x', function(d, i){ return d.xOffset+ (cfg.w/2-radius2)+getHorizontalPosition(i, radius2, cfg.factorLegend); })
            .attr('y', function(d, i){ return d.yOffset+ (cfg.h/2-radius2)+getVerticalPosition(i, radius2, cfg.factorLegend)+cfg.verticalOffset; });
          }
        }
        ...
        .attr('points',function(d) {
          return d.axes.map(function(p) {
            return [p.x, p.y + cfg.verticalOffset].join(',');
          }).join(' ');
        })
        .each('start', function() {
        ...
        if(cfg.circles && cfg.radius) {
          ...
          .attr('cy', function(d) {
            return d[0].y + cfg.verticalOffset;
          })
          .each('start', function() {
            d3.select(this).classed('d3-enter', 0); // trigger css transition
          });
